...
def getNewQuestion(request):
    try:
        ...
        if level=="Level1":
            ...
            return {"state":"success","question":serializationQuestion(random.choice(allLevelQuestion)[1],"Level2",commonUser)}
        if level=="Level2":
            alreadyDoneID=History.objects.filter(commonUser=commonUser).values_list("questionID")
            allLevelQuestion=Level3.objects.exclude(question__in=alreadyDoneID).values_list("example")
            return {"state":"success","question":serializationQuestion(random.choice(allLevelQuestion)[1],"Level3",commonUser)}
        if level=="Level3":
            alreadyDoneID=History.objects.filter(commonUser=commonUser).values_list("questionID")
            allLevelQuestion=Level4.objects.exclude(question__in=alreadyDoneID).values_list("example")
            return {"state":"success","question":serializationQuestion(random.choice(allLevelQuestion)[1],"Level4",commonUser)}
        if level=="Level4":
            alreadyDoneID=History.objects.filter(commonUser=commonUser).values_list("questionID")
            allLevelQuestion=Level4.objects.exclude(question__in=alreadyDoneID).values_list("example")
            return {"state":"success","question":serializationQuestion(random.choice(allLevelQuestion)[1],"Level4",commonUser)}
